...
from .svd_symeig import SVDSYMEIG
from .svd_arnoldi import SVDSYMARNOLDI, SVDARNOLDI
from .svd_rsvd import RSVD
from .svd_af import SVDAF
...
def truncated_svd_gesdd(M, chi, abs_tol=1.0e-14, rel_tol=None, ad_decomp_reg=1.0e-12,\
    ...
    norm = (torch.flip(S, (0,)) ** 2).cumsum(0) / (S**2).sum(0)
    # You get the first index where the constraint is broken,
    # so you need to stop an index before
    cut = torch.nonzero(norm > rel_tol)[0]

    chi_now = len(S)
    chi_by_norm = len(S) - cut[0] if len(cut) > 0 else chi_now
    rank = min(chi_now, min(chi, chi_by_norm))

    singvals_kept = S[:rank]
    singvals_cutted = S[rank:]
